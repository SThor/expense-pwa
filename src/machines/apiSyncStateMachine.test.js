...
describe('apiSyncStateMachine', () => {
  ...
  describe('START_SYNC Transitions', () => {
    ...
    it('should handle YNAB-only target correctly', () => {
      const machineWithMockServices = apiSyncStateMachine.provide({
        actors: {
          ...
          submitSettleup: fromPromise(() => Promise.resolve('SettleUp Success')),
        },
      });
    ...
    it('should handle SettleUp-only target correctly', () => {
      ...
      expect(snapshot.matches({ syncing: { settleup: 'submitting' } })).toBe(true);
  ...
  describe('Success Scenarios', () => {
    it('should reach success state when all targeted APIs succeed', async () => {
      ...
      await vi.waitFor(() => {
        const snapshot = actor.getSnapshot();
        return snapshot.matches('success') || snapshot.matches('error') || snapshot.matches('partialSuccess');
      }, { timeout: 1000 });
  ...
  describe('Error Scenarios', () => {
    it('should reach error state when single targeted API fails', async () => {
      const machineWithMockServices = apiSyncStateMachine.provide({
        actors: {
          submitYnab: fromPromise(() => Promise.reject(new Error('YNAB Failed'))),
          submitSettleup: fromPromise(() => Promise.resolve('SettleUp Success')),
        },
      });
      ...
      await vi.waitFor(() => {
        const snapshot = actor.getSnapshot();
        return snapshot.matches('error') || snapshot.matches('success') || snapshot.matches('partialSuccess');
      }, { timeout: 1000 });
    ...
    it('should reach partialSuccess state when one of multiple APIs fails', async () => {
      const machineWithMockServices = apiSyncStateMachine.provide({
        actors: {
          submitYnab: fromPromise(() => Promise.resolve('YNAB Success')),
          submitSettleup: fromPromise(() => Promise.reject(new Error('SettleUp Failed'))),
        },
      });
      ...
      await vi.waitFor(() => {
        const snapshot = actor.getSnapshot();
        return snapshot.matches('partialSuccess') || snapshot.matches('error') || snapshot.matches('success');
      }, { timeout: 1000 });
    ...
    it('should reach partialSuccess when SettleUp fails but YNAB succeeds', async () => {
      ...
      expect(finalSnapshot.context.errors.ynab).toBe(null);
      expect(finalSnapshot.context.errors.settleup).toBeInstanceOf(Error);
      expect(finalSnapshot.context.errors.settleup.message).toBe('SettleUp Failed');
  ...
  describe('Retry Logic', () => {
    it('should retry all APIs when in error state', async () => {
      ...
      expect(retrySnapshot.context.results).toEqual({ ynab: null, settleup: null });
      expect(retrySnapshot.context.errors).toEqual({ ynab: null, settleup: null });
  ...
  describe('Reset Functionality', () => {
    it('should reset to idle from success state', async () => {
      ...
      expect(resetSnapshot.context.results).toEqual({ ynab: null, settleup: null });
      expect(resetSnapshot.context.errors).toEqual({ ynab: null, settleup: null });
