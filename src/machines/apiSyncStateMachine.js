...
export const apiSyncStateMachine = createMachine(
            ...
            actions: assign({
              formState: ({ context }) => ({
                ...
                target: {
                  ...
                  settleup:
                    context.formState.target.settleup &&
                    !!context.errors.settleup,
                },
              }),
              results: { ynab: null, settleup: null },
              errors: { ynab: null, settleup: null },
  ...
  },
  {
    guards: {
      ...
      settleupTargeted: ({ context }) =>
        context.formState?.target?.settleup || false,
      allTargetsSucceeded: ({ context }) => {
        // An API is "OK" if: not targeted OR (has result AND no error)
        const ynabOk =
          !context.formState?.target?.ynab ||
          (!!context.results.ynab && !context.errors.ynab);
        const settleupOk =
          !context.formState?.target?.settleup ||
          (!!context.results.settleup && !context.errors.settleup);
