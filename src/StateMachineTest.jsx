...
import ApiSyncStepper from "./components/ApiSyncStepper.jsx";
...
function StateMachineTest() {
  ...
  const getStatePath = (state) => {
    ...
    if (state.matches("syncing")) {
      ...
      // Check YNAB substate
      if (state.matches({ syncing: { ynab: "checking" } }))
        paths.push("syncing.ynab.checking");
      if (state.matches({ syncing: { ynab: "skipped" } }))
        paths.push("syncing.ynab.skipped");
      if (state.matches({ syncing: { ynab: "submitting" } }))
        paths.push("syncing.ynab.submitting");
      if (state.matches({ syncing: { ynab: "success" } }))
        paths.push("syncing.ynab.success");
      if (state.matches({ syncing: { ynab: "error" } }))
        paths.push("syncing.ynab.error");

      // Check SettleUp substate
      if (state.matches({ syncing: { settleup: "checking" } }))
        paths.push("syncing.settleup.checking");
      if (state.matches({ syncing: { settleup: "skipped" } }))
        paths.push("syncing.settleup.skipped");
      if (state.matches({ syncing: { settleup: "submitting" } }))
        paths.push("syncing.settleup.submitting");
      if (state.matches({ syncing: { settleup: "success" } }))
        paths.push("syncing.settleup.success");
      if (state.matches({ syncing: { settleup: "error" } }))
        paths.push("syncing.settleup.error");
    }
      ...
      >
        ...
        <ul>
          {getStatePath(state).map((path) => (
            <li key={path} style={{ color: "#0066cc", fontWeight: "bold" }}>
              {path}
            </li>
          ))}
        </ul>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ marginBottom: "10px" }}>
          ...
          <button
            onClick={() => {
              if (ynabResolver) {
                ynabResolver.reject(
                  new Error("YNAB API error: Network timeout"),
                );
                setYnabResolver(null);
              }
            }}
        ...
        <div>
          ...
          <button
            onClick={() => {
              if (settleupResolver) {
                settleupResolver.reject(
                  new Error("Error adding transaction: Invalid permissions"),
                );
                setSettleupResolver(null);
              }
            }}
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <button
          ...
          style={{
            ...
            padding: "10px 20px",
            backgroundColor:
              !state.matches("idle") && !state.matches("syncing")
                ? "#6c757d"
                : "#f8f9fa",
            color:
              !state.matches("idle") && !state.matches("syncing")
        ...
        <button
          ...
          style={{
            padding: "10px 20px",
            backgroundColor: state.matches("partialSuccess")
              ? "#17a2b8"
              : "#f8f9fa",
