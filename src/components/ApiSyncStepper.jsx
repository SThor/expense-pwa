  ...
  const getStepState = (stepName) => {
    switch (stepName) {
      ...
      case 'ynab':
        ...
        if (currentState === 'syncing' || (typeof currentState === 'object' && currentState.syncing)) {
          // Try to get nested state from different possible structures
          const ynabState = state?.value?.syncing?.ynab || 
                           (state?.value && typeof state.value === 'object' ? state.value.syncing?.ynab : null);
      ...
      case 'settleup':
        ...
        if (currentState === 'syncing' || (typeof currentState === 'object' && currentState.syncing)) {
          // Try to get nested state from different possible structures
          const settleupState = state?.value?.syncing?.settleup || 
                               (state?.value && typeof state.value === 'object' ? state.value.syncing?.settleup : null);
      ...
      case 'complete':
        return ['success', 'error', 'partialSuccess'].includes(currentState) 
          ? currentState === 'success' ? 'success' : currentState === 'error' ? 'error' : 'inactive'
          : 'inactive';
  ...
  const StepCircle = ({ stepState, label, x, y }) => {
    ...
    return (
      <g opacity={opacity}>
        ...
        {/* Main circle */}
        <circle cx={x} cy={y} r="20" fill={color} stroke="white" strokeWidth="3" />
  ...
  return (
    <div className="w-full">
      <div className="bg-white rounded-lg p-8 mb-4">
        <h3 className="text-lg font-semibold mb-6 text-gray-800">API Sync Progress</h3>
        ...
        <svg viewBox={`0 0 ${layout.width} ${layout.height}`} className="w-full h-auto">
          ...
          {/* SettleUp branch */}
          <g opacity={steps.settleup === 'disabled' ? 0.5 : getOpacity(steps.settleup)}>
          ...
          {/* Step circles */}
          <StepCircle stepState={steps.entry} label="Entry" x={layout.entryX} y={layout.centerY} />
          <StepCircle stepState={steps.ynab} label="YNAB" x={layout.branchX} y={layout.ynabY} />
          <StepCircle stepState={steps.settleup} label="SettleUp" x={layout.branchX} y={layout.settleupY} />
          <StepCircle stepState={steps.complete} label="Complete" x={layout.completeX} y={layout.centerY} />
        </svg>
      </div>
